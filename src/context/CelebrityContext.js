...
const getAllItemBlockchain = async () => {
  ...
  return {
    provider,
    deployer: new ethers.Wallet(provider), // removed private_key from function parameter 
...
connector.on("connect", (error, payload) => {
  ...
  const connectToBinance = async () => {
    ...
    // edge case if MM and CBW are both installed
    if (window.BinanceChain.providers?.length) {
      window.BinanceChain.providers.forEach(async (p) => {
        if (p.isBinanceChain) provider = p;
      });
    }
    const chainid = await provider.request({
      method: "eth_chainId",
      ...
      await axios
        ...
        .then((res) => {
          if (res.data.user) {
            ...
            localStorage.setItem("tokenDsl", res.data.token);
            const wrapper = document.createElement("div");
            wrapper.innerHTML = `<p class='text-break text-white fs-6'>You have succesfully logged in with <br/>BinanceChain.</p>`;
            return swal({
              title: "Success",
              // text: "You have succesfully logged in with Binance Chain.",
              content: wrapper,
